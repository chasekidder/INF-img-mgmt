...
def find_tag(folder_path, tag):
    ...
    subprocess.call("exiftool -m -q -q -T -r -filename -if '$FileModifyDate ' -if '$keywords =~ /" +
                    tag + "/' " + folder_path + " > match_tag.txt", shell=True)
...
def check_tags(folder_path):
    ...
    # Check for all files in the folder that have a rating of 3 stars
    # and no tags
        # Command:
        # exiftool -T -r -filename -if 'not $keywords'
        # -if '$rating eq 3' [PATH GOES HERE]

    subprocess.call("exiftool -m -q -q -T -r -filename -if 'not $keywords' -if '$rating eq 3' " +
                    folder_path + " > untagged_files.txt", shell=True)

    # TODO: Add modifydate check to command w/ configurable time setting
    # to check against

    # TODO: See if I can change over to popen instead of subprocess.call
    # 7.18.19 - Can't seem to get the conditional to work right with
    # the seperate argument format

    #test_file = open('test_file.txt','w')
    # process = Popen(["exiftool","-v1", "-filename", "-if","\'not
    # $keywords\' ", folder_path], stdin=PIPE, stdout=test_file)
...
def crop_image(img_toBeCropped, new_dimensions):
    ...
    # centering is percentage taken from each side that is being cropped
    # ie. 50% from top and bottom if the height is being cropped
    img_toBeCropped = ImageOps.fit(
        img_toBeCropped, new_dimensions, centering=(0.5, 0.5))
...
def add_tag(file_path, tag):
    ...
    logging.debug("Adding \"" + tag + "\" to: " + file_path)

    # Add keyword to file using exiftool
    process = Popen(['exiftool', '-m', '-q', '-q', '-keywords+=' +
                     tag, file_path, "-overwrite_original"], stdin=PIPE, stdout=PIPE)
...
def remove_tag(file_path, tag):
    ...
    logging.debug("Removing \"" + tag + "\" from: " + file_path)
...
def copy_rating(file_path, new_file_path):
    ...
    process = Popen(['exiftool', '-m', '-q', '-q', "−tagsFromFile", file_path,
                     '-rating>rating', "-overwrite_original", new_file_path], stdin=PIPE, stdout=PIPE)
